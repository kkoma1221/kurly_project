...
export default function Sub1Component() {
    ...
    //console.log( location.pathname.split('/') ); // '/'를 기준으로 문자를 나눔 => 배열로 나옴 ['', 'sub1']
    //console.log( location.pathname.split('/')[1] ); // 2번째 문자만 뽑음
    ...
    React.useEffect(()=>{
        ...
        if(state.필터.length > 0){ // 배열의 개수(길이 length)가 0보다 클 때 
            sessionStorage.setItem('KURLY_SUB1_FILTER_ITEM', JSON.stringify(state.필터)); // storage저장소는 반드시 문자열만 저장됨 배열 => 문자열 JSON.stringify
        }
    },[state.필터]);
        ...
        .then((res)=>{
            ...
            if(res.status === 200){
                ...
                if(sessionStorage.getItem('KURLY_SUB1_FILTER_ITEM')!==null){
                    result = JSON.parse(sessionStorage.getItem('KURLY_SUB1_FILTER_ITEM')); // 배열객체로 변환
                }
                setState({
                    ...state,
                    타이틀: {
